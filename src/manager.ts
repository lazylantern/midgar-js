...
export default class MidgarManager {
  ...
  public start() {
    ...
    this.api.checkAppIsEnabled().then(response => {
      if (response.ok) {
  ...
  private processBatch() {
    ...
    if (batch.length > 0) {
      this.api.uploadBatch(batch).then(uploadResponse => {
        if (__DEV__) {
  ...
  private checkSessionIdValidity() {
    return (
      new Date().getTime() - this.timeOfLastBackgroundEvent <
  ...
  private getActiveRouteName(navigationState: NavigationState) {
    ...
    return this.getActiveRouteNameFromRoute(
      navigationState.routes[navigationState.index]
    );
  }

  private getActiveRouteNameFromRoute(
    ...
    if (navigationRoute.routes) {
      return this.getActiveRouteNameFromRoute(
        navigationRoute.routes[navigationRoute.index]
      );
    }
